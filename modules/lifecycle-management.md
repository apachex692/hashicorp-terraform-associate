...
The `plan` command is used to generate an execution plan. It compares the current state of your infrastructure as described by your Terraform configuration files (`*.tf`) with the desired state defined in those files. It then generates a set of actions required to achieve the desired state.
...
*Note that Terraform will apply the changes without your approval in this case. Usually you'd be prompted before applying changes.*
